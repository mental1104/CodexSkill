...
Use this skill to help a human understand code through one concrete business journey and, when requested, review maintainability and design concerns through an interactive queue.

This workflow is intended for cases where:

- AI generated or modified more code than the user can immediately absorb;
...
- the user wants review concerns identified without immediately changing code;
- the user prefers fast, single-point answers and does not want unsolicited expansion.
...
- 代码走读、源码走读、链路走读、业务走读；
- 从入口读到业务闭环；
- 走读某个微服务、目录、分支、PR 或功能；
- 先建立代码上下文，再边读边问；
- 根据可维护性规则扫描问题；
- 一次一个问题地审阅代码；
- 对粘贴的代码片段自动定位并解释其在当前链路中的作用；
- review a repository, branch, directory, service, feature path, lifecycle, pull request, or code fragment conversationally.
...
4. **One concern at a time.** Report candidate counts and severity distribution, but expand only the current concern.
5. **Evidence before judgment.** Every finding requires a real path, symbol, code path, diff, test, or contract.
...
10. **Do not proactively teach everything.** Explain language or library details only when the user asks or when they are necessary to understand the current answer.
...
- `references/maintainability-gates.md`: load before scanning, prioritizing, or deeply discussing maintainability and design concerns. It contains G01-G08, function-extraction rules, design-pattern criteria, false-positive controls, and the Go profile.
- `references/chatgpt-project-prompt.md`: ready-to-copy instructions for an XDLP ChatGPT Project.

A simple syntax or single-point explanation should not pay the cost of loading the complete gate reference unless a concrete concern must enter the queue.
...
If the user provides a clear business journey but not an entrypoint, locate it from routing, command registration, server bootstrap, consumer registration, scheduled-job registration, or symbol search. Do not force the user to provide a path that repository inspection can resolve.

## Pasted Code Fragment

A pasted fragment inherits the active session. The user should not need to manually supply its path, line number, function name, and every referenced function.
...
Context building is complete when the assistant can answer from evidence:
...
Do not require a long-running server or consumer to have a normal business endpoint. Reconstruct two separate cycles when applicable.
...
The business cycle and process lifecycle are separate review objects and must not be mixed into one vague flow.

## Context Output

When context is first established, output a compact map and stop before scanning concerns unless the user explicitly asked to do both in one operation.
...
Before scanning, read `references/maintainability-gates.md`.
...
Every queued concern must include real evidence, survive the gate's exceptions, and state the smallest suggestion direction. Do not create a concern merely to satisfy a desired count.

A design-pattern recommendation is allowed only after identifying the real change axis, stable responsibility, current extension cost, simpler alternative, pattern option, and new abstraction cost.
...
Bind the ledger to repository, ref, commit or PR head SHA, frozen scope, user journey, and scan version.
...
For a simple learning question, answer directly without forcing the full concern template unless it has entered the queue.

## User Operations

Support natural-language operations:

- `接受`：record the concern as valid within current evidence;
...
When the user explicitly asks to fix selected concerns:

1. freeze selected concern IDs and scope;
...
4. preserve current behavior unless an accepted concern requires behavior change;
...
2. refresh only affected context paths;
...
- If the entrypoint cannot be uniquely identified, show the smallest plausible set and explain the distinguishing event.
...
- If the scope is too broad, choose the smallest business cycle implied by the request rather than scanning everything.
- If no concern survives evidence and exceptions, say so. Do not manufacture queue items.
...
10. remain read-only unless the user explicitly changes mode;
...
When the user pastes `runAuthenticated` without a path and asks where it sits in the lifecycle:

- search the active repository and scope using its function name and distinctive calls;
- bind the matching file and symbol;
- explain that it is process-level orchestration rather than a per-request business handler;
- do not require the user to copy paths, lines, and every callee.
