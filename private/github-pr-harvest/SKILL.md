...
description: Build a repository-grounded GitHub Pull Request payload with a focused Chinese title and high-signal body. Use whenever the user asks to 写 PR、整理 PR、提 PR、提交 PR、创建 PR、开 PR、完成 PR or equivalent wording. This skill owns PR content semantics; all GitHub-specific reads and remote mutations are delegated to github-operations.
---
...
If local repository access is unavailable and the evidence exists only on GitHub, request the minimum required branch, commit, diff, PR, Issue, or check evidence through `github-operations`. This skill must not invoke GitHub tools directly.
...
For these, generate the title and body with this skill, then return an authorized publication request to `github-operations`.
...
1. use this skill to finish PR evidence, title, body, and target-base requirements;
2. package the requested branch/commit/push/PR mutation for `github-operations`;
3. let `github-operations` verify the repository remote and explicit write authorization before any remote mutation;
4. consume the gateway result and report branch, commit, target branch, tests, PR link, and any remaining limitations.

This skill owns PR content quality. `github-operations` owns every GitHub remote read/write and any `git push`.
